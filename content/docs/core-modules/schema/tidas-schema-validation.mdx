...
CLI 的 `tidas validate`。它离线使用当前安装版本内嵌且经过完整性校验的
JSON Schema、ILCD XSD 和规则资源，不依赖外部源码目录或 Python 环境。用
`tidas version --format json` 记录实际二进制版本和资产指纹；发布信息以
[`tidas-toolkit` Releases](https://github.com/tiangong-lca/tidas-toolkit/releases/latest) 为准。
...
`--schema-only` 只检查 Schema 层，不执行完整包、引用或转换 profile 检查，
因此不能证明数据可转换为 eILCD，也不能作为完整符合性证据。
