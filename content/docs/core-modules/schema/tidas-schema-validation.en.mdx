...
integrity-locked JSON Schema, ILCD XSD, and rules assets embedded in the installed
version, without an external source tree or Python environment. Record the actual
binary and asset fingerprints with `tidas version --format json`; use
...
`--schema-only` checks the Schema layer only. It skips full package, reference,
and conversion-profile checks, so it does not prove eILCD convertibility and is
not complete conformance evidence.
...
[Data Structure (JSON Schema)](/en/docs/core-modules/schema/schema-content/json-schema-contact/). For
installation, platform support, and legacy-command migration, see the
[unified TIDAS CLI](/en/docs/tool/tidas-tool-intro/).
