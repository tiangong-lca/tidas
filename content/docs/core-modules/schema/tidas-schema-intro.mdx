...
TIDAS Schema 由 17 个核心 JSON Schema 文件组成，每个文件描述了一个具体数据模块，具体内容详见 [数据结构（JSON Schema）](/zh/docs/core-modules/schema/schema-content/json-schema-contact/)。

如需了解如何使用 JSON Schema 进行数据验证和格式转换，请参考 [数据结构验证](/zh/docs/core-modules/schema/tidas-schema-validation/)。
